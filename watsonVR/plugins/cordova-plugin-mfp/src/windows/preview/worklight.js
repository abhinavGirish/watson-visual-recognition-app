        ...
        if (WL.Client.getEnvironment() === WL.Env.ANDROID) {
            ...
            var processResponse = function(transport) {
                ...
                if (transport.status === 409) {
                    ...
                    WLAuthorizationManager.__cacheScopeByResource(transport, '').always(function() {
                        resendRequest(++attempt, conflictAttemptCounter);
                    });
                }
            };
...
WLAuthorizationManager.__getClientIdOrInvokeRegistration = function() {
  ...
  .then(
    ...
    function (error) {
      ...
      if (devicedata.id==WL.StaticAppProps.APP_ID && devicedata.clientPlatform==WL.StaticAppProps.ENVIRONMENT && devicedata.version == WL.StaticAppProps.APP_VERSION && devicedata.model == device.model && devicedata.os == 'Windows '+device.version) {
...
WLAuthorizationManager.__setClientRegisteredData = function() {
    ...
    devicedata['hardware'] = device.model;
    devicedata['platform'] = 'Windows '+device.version;
